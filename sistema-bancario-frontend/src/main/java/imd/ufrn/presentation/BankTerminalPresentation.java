...
public class BankTerminalPresentation {
    @Autowired
    ApiBankCommunicationService apiBankCommunication;
    ...
    public void realizeTransfer() {
        ...
        boolean isSuccess = apiBankCommunication.transfer(originAccountNumber, destinationAccountNumber,
                valueToTransfer);
        if (isSuccess) {
    ...
    public void consultAccount() {
        ...
        if (account != null) {
            System.out.println("Tipo de Conta: " + (account.getAccountType() == 2 ? "Conta Poupança"
                    : (account.getAccountType() == 3 ? "Conta Bônus" : "Conta Normal")));
